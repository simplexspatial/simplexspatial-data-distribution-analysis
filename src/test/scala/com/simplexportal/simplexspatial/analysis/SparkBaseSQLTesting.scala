...
/**
  ...
  */
trait SparkBaseSQLTesting extends SparkBaseTesting { self: Suite =>
  ...
  lazy implicit val sparkSession: SparkSession = {
    val session = SparkSession
      ...
      .getOrCreate()
    session.sparkContext.setLogLevel(errorLevel)

    session
  }

  lazy implicit val sparkContext: SparkContext = sparkSession.sparkContext
}
